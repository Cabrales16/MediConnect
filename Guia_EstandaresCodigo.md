...
- **Variables y métodos en Python**:
    ...
    Se debe utilizar ``snake_case`` (todo en minúsculas y separado por guion bajo), por ejemplo: ``mi_perfil()``
- **Clases en Python**:

Se debe usar ``PascalCase`` (cada palabra inicia con mayúscula), por ejemplo: ``EstadoCitaSeguridad``
- **Variables y funciones en React**:

Se debe usar ``camelCase`` (la primera palabra en minúscula y las siguientes con mayúscula), por ejemplo: ``miPerfil``
...
**3. Mal manejo de espacios en blanco**:
```py
