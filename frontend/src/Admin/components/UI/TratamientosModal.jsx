...
export default function TratamientosModal({ isOpen, onClose, user, tratamientos = [] }) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-2xl">
        <h2 className="text-lg font-semibold mb-4">Tratamientos de {user?.nombre}</h2>
        ...
        <div className="max-h-80 overflow-auto border rounded p-3">
          {tratamientos.length > 0 ? (
            ...
            <p className="text-gray-400">No hay tratamientos registrados</p>
          )}
        </div>
