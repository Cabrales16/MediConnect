// src/components/Navbar/NotificationsButton.jsx
import React, { useState, useRef, useEffect } from "react";

/* IMPORTS: reemplaza por tus archivos reales en ./NavbarIcons/ */
...
export default function NotificationsButton() {
  ...
  const [hoveredTrashId, setHoveredTrashId] = useState(null); // id cuya papelera está en hover
        ...
        <div className="absolute right-0 mt-2 w-80 z-50">
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <ul className="max-h-64 overflow-auto">
              {notifications.length === 0 ? (
                <li className="p-4 text-sm text-gray-500">No hay notificaciones</li>
              ) : (
                notifications.map((n) => (
                  <li key={n.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50">
                    {/* icono de la notificación (tú provees estos) */}
            ...
            <div className="p-4 flex justify-center">
              <button
                ...
                className={`px-4 py-2 rounded-md ${notifications.length === 0 ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-200"} transition text-sm`}
